...
class Mertracking(object):
    ...
    def __init__(self, friendly_mode=True):
        ...
        self.mh = adafruit_motor.stepper
        atexit.register(self.__turn_off_motors)

        # Stepper motor 1
        self.sm_x = self.mh.stepper1(200, 1)      # 200 steps/rev, motor port #1
        self.sm_x.setSpeed(5)                       # 5 RPM
        self.current_x_steps = 0

        # Stepper motor 2
        self.sm_y = self.mh.stepper2(200, 2)      # 200 steps/rev, motor port #2
        self.sm_y.setSpeed(5)                       # 5 RPM
        self.current_y_steps = 0

        # Relay
        DigitalOutputDevice.setmode(DigitalOutputDevice.BCM)
        DigitalOutputDevice.setup(RELAY_PIN, DigitalOutputDevice.OUT)
        DigitalOutputDevice.output(RELAY_PIN, DigitalOutputDevice.LOW)
    ...
    @staticmethod
    def fire():
        DigitalOutputDevice.output(RELAY_PIN, DigitalOutputDevice.HIGH)
        time.sleep(1)
        DigitalOutputDevice.output(RELAY_PIN, DigitalOutputDevice.LOW)
    ...
    def move_forward(motor, steps):
        ...
        :return:
        """
        motor.step(steps, stepper.FORWARD,  stepper.MICROSTEP)
    ...
    def move_backward(motor, steps):
        ...
        :return:
        """
        motor.step(steps, stepper.BACKWARD, stepper.MICROSTEP)
    ...
    def __turn_off_motors(self):
        ...
        :return:
        """
        self.mh.stepper1.release()
        self.mh.stepper2.release()
